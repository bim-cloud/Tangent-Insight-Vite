...
export function AttendanceScreen({ data, onPickUser }) {
  ...
  const present = people.filter((p) => p.attStatus === "ON_TIME" || p.attStatus === "LATE");
  const absent = people.filter((p) => p.attStatus === "ABSENT");
  const onLeave = people.filter((p) => p.attStatus === "ON_LEAVE");
  ...
  return (
    <motion.div variants={staggerGrid} initial="initial" animate="animate">
      <motion.div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))" }} variants={staggerGrid}>
        ...
        <ATile icon="UserX" label="Absent" value={absent.length} grad="var(--grad-navy)" />
        <ATile icon="Plane" label="On leave" value={onLeave.length} grad="var(--grad-cyan)" />
              ...
              return (
                <tr key={p.id} className="click" onClick={() => onPickUser?.(p)} style={{ cursor: "pointer" }}>
                  ...
                  <td>{
                    ...
                    p.attStatus === "LATE" ? <Pill tone="warning" dot>late</Pill> :
                    <Pill tone="success" dot>on time</Pill>
                  }</td>
...
function BreakBreakdown({ personId }) {
  const [breaks, setBreaks] = useState(null);
  useEffect(() => {
    let alive = true;
    const today = new Date().toISOString().slice(0, 10);
    rest("rpc/breaks_for_day", "", null, { method: "POST", body: { p_person: personId, p_day: today } })
      .then((r) => { if (alive) setBreaks(Array.isArray(r) ? r : []); })
      .catch(() => { if (alive) setBreaks([]); });
    return () => { alive = false; };
  }, [personId]);
  if (breaks === null) return null;
  const total = breaks.reduce((a, b) => a + (b.minutes || 0), 0);
  const fmtT = (iso) => iso ? new Date(iso).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }) : "—";
  return (
    <div className="surface-solid" style={{ ...card, marginBottom: 16 }}>
      <div className="between" style={{ marginBottom: 8 }}>
        <span className="micro">Today's breaks</span>
        <span className="muted" style={{ fontSize: 11 }}>{total >= 60 ? Math.floor(total / 60) + "h " + (total % 60) + "m" : total + "m"} total</span>
      </div>
      {breaks.length === 0 ? <div className="muted" style={{ fontSize: 11 }}>No breaks recorded today.</div> : (
        <div className="col gap-1">
          {breaks.map((b, i) => (
            <div key={i} className="between" style={{ padding: "5px 9px", borderRadius: 7, background: "rgb(var(--bg-sunken))" }}>
              <span className="muted" style={{ fontSize: 11 }}>{fmtT(b.break_start)} → {fmtT(b.break_end)}</span>
              <span className="tabular" style={{ fontSize: 11, fontWeight: 600 }}>{b.minutes}m</span>
            </div>
          ))}
        </div>
      )}
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
            ...
            {cells.map((d, i) => {
              ...
              const onLeave = rec && rec.status === "ON_LEAVE";
              const bg = onLeave ? "rgb(var(--accent)/0.18)" : !present ? "rgb(var(--bg-sunken))" : late ? "rgb(var(--warning)/0.18)" : "rgb(var(--success)/0.18)";
              const fg = onLeave ? "rgb(var(--accent))" : !present ? "rgb(var(--fg-muted))" : late ? "rgb(var(--warning))" : "rgb(var(--success))";
              const tip = onLeave ? "On leave"
                : rec && present ? `${fmtClock(rec.first_in)}–${fmtClock(rec.last_out)} · active ${fmtH(rec.active_minutes)}${rec.break_minutes ? " · break " + fmtH(rec.break_minutes) : ""}${rec.overtime_minutes ? " · OT " + fmtH(rec.overtime_minutes) : ""}${late ? " · late" : ""}`
                : "Absent";
              return (
                <div key={i} title={tip}
                  ...
                  {onLeave && <div style={{ fontSize: 7, opacity: 0.8 }}>leave</div>}
                </div>
              );
            })}
          </div>
